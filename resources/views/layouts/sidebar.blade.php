    ...
    <nav class="flex-1 overflow-y-auto py-6 px-4 space-y-1.5 sidebar-scroll" x-data="{ 
        ...
        <div class="relative">
            ...
            <div x-show="openAdmin" x-collapse class="pr-6 space-y-1 mt-1 overflow-hidden">
                <div class="border-r-2 border-gray-800 space-y-1 py-1 mr-3">
                    ...
                    <a href="{{ route('admin.online-users.index') }}" class="group/sub flex items-center gap-3 px-4 py-2.5 text-sm rounded-lg transition-colors {{ request()->routeIs('admin.online-users.index') ? 'bg-red-500/10 text-white' : 'text-gray-500 hover:text-gray-300 hover:bg-white/5' }}">
                        <span class="w-1.5 h-1.5 rounded-full {{ request()->routeIs('admin.online-users.index') ? 'bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.6)]' : 'bg-green-500 animate-pulse' }} transition-all"></span>
                        متصلون الآن
                    </a>
        ...
        <script>
            function showWhatsAppRedirect() {
                ...
                // بعد 5 ثواني، التحويل لـ WhatsApp (في نفس التاب لتجنب حظر المتصفح)
                setTimeout(function() {
                    window.location.href = 'https://wa.me/201558551073?text=مرحباً، أرغب في تفعيل الوصول لخدمة واتساب API';
                }, 5000);
            }
        </script>
        @endif
