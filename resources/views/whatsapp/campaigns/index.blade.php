...
<div class="space-y-8 direction-rtl">
    ...
    <!-- Campaigns List -->
    <div class="bg-[#1e1e24]/60 backdrop-blur-md border border-gray-800 rounded-2xl overflow-hidden relative min-h-[400px]">

        <!-- Table Header Gradient -->
        <div class="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 opacity-50"></div>

        <div class="overflow-x-auto custom-scrollbar">
            <table class="w-full text-right">
                <thead>
                    <tr class="border-b border-gray-800/80 bg-gray-900/40">
                        <th class="px-6 py-5 text-xs font-bold text-gray-400 uppercase tracking-wider">اسم الحملة</th>
                        <th class="px-6 py-5 text-xs font-bold text-gray-400 uppercase tracking-wider">المجموعة المستهدفة</th>
                        <th class="px-6 py-5 text-xs font-bold text-gray-400 uppercase tracking-wider">حالة الحملة</th>
                        <th class="px-6 py-5 text-xs font-bold text-gray-400 uppercase tracking-wider w-1/4">التقدم الفعلي</th>
                        <th class="px-6 py-5 text-xs font-bold text-gray-400 uppercase tracking-wider">الإعدادات</th>
                        <th class="px-6 py-5 text-xs font-bold text-gray-400 uppercase tracking-wider text-center">التحكم</th>
                    </tr>
                </thead>
                    ...
                    <tr class="hover:bg-white/5 transition-colors group">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-lg bg-indigo-500/10 flex items-center justify-center text-indigo-400 group-hover:bg-indigo-500 group-hover:text-white transition-all duration-300 shadow-sm border border-indigo-500/20">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z" />
                                    </svg>
                                </div>
                                <div>
                                    <div class="font-bold text-white text-base group-hover:text-indigo-300 transition-colors">{{ $campaign->campaign_name }}</div>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex flex-col">
                                <span class="text-gray-300 font-medium">{{ $campaign->contact->contact_name ?? 'مجموعة محذوفة' }}</span>
                                <span class="text-xs text-gray-500 font-mono bg-gray-800/50 px-2 py-0.5 rounded w-fit mt-1 border border-gray-700">ID: {{ $campaign->instance_id }}</span>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4">
                            ...
                            'sending' => 'bg-blue-500/10 text-blue-500 border-blue-500/20 animate-pulse',
                            ...
                            @endphp
                            <span class="px-3 py-1.5 rounded-lg text-xs font-bold border flex items-center gap-2 w-fit {{ $statusClasses[$currentStatus] ?? 'bg-gray-700 text-gray-300' }}">
                                <span class="w-1.5 h-1.5 rounded-full {{ str_replace('/10', '', $statusClasses[$currentStatus] ?? 'bg-gray-500') }} animate-pulse"></span>
                            ...
                            <div class="flex flex-col gap-2">
                                <div class="flex justify-between items-center text-[11px] font-bold">
                                    <div class="flex gap-3">
                                        <span class="text-green-400 flex items-center gap-1"><span class="w-1.5 h-1.5 rounded-full bg-green-500"></span> {{ $campaign->sent_count }}</span>
                                        <span class="text-red-400 flex items-center gap-1"><span class="w-1.5 h-1.5 rounded-full bg-red-500"></span> {{ $campaign->failed_count }}</span>
                                    </div>
                                    <span class="text-gray-500">من {{ $campaign->total_numbers }}</span>
                                </div>

                                <div class="w-full h-2 bg-gray-800 rounded-full overflow-hidden flex relative">
                                    <!-- Sent Progress -->
                                    <div class="h-full bg-gradient-to-r from-green-600 to-green-400 shadow-[0_0_10px_rgba(34,197,94,0.5)] transition-all duration-500"
                                        style="width: {{ $campaign->total_numbers > 0 ? ($campaign->sent_count / $campaign->total_numbers) * 100 : 0 }}%"></div>

                                    <!-- Failed Progress -->
                                    <div class="h-full bg-red-500/50 transition-all duration-500"
                                        style="width: {{ $campaign->total_numbers > 0 ? ($campaign->failed_count / $campaign->total_numbers) * 100 : 0 }}%"></div>
                                </div>

                                <div class="text-right text-[10px] text-gray-500 font-mono">
                                    {{ $campaign->total_numbers > 0 ? round((($campaign->sent_count + $campaign->failed_count) / $campaign->total_numbers) * 100) : 0 }}% منجز
                                </div>
                            </div>
                        </td>
                        ...
                        <!-- Settings (Delays) -->
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-2 text-gray-400 bg-gray-900/50 px-3 py-1.5 rounded-lg border border-gray-800 w-fit mb-1">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-indigo-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                                <span class="text-xs font-mono">{{ $campaign->min_delay }} - {{ $campaign->max_delay }} ثانية</span>
                            </div>
                            @if($campaign->batch_size > 0)
                            <div class="flex items-center gap-2 text-amber-400/80 bg-amber-900/10 px-3 py-1.5 rounded-lg border border-amber-800/30 w-fit">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                                </svg>
                                <span class="text-[11px] font-mono">{{ $campaign->batch_size }} رسالة / {{ $campaign->batch_sleep }} د</span>
                            </div>
                            @endif
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                ...
                                <form action="{{ route('whatsapp.campaigns.status', $campaign->id) }}" method="POST">
                                    ...
                                    <button type="submit" class="w-8 h-8 rounded-lg bg-green-500/10 text-green-500 hover:bg-green-500 hover:text-white flex items-center justify-center transition-all shadow-sm hover:shadow-green-500/30" title="بدء / استئناف">
                                ...
                                <form action="{{ route('whatsapp.campaigns.status', $campaign->id) }}" method="POST">
                                    ...
                                    <button type="submit" class="w-8 h-8 rounded-lg bg-yellow-500/10 text-yellow-500 hover:bg-yellow-500 hover:text-white flex items-center justify-center transition-all shadow-sm hover:shadow-yellow-500/30" title="إيقاف مؤقت">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                                ...
                                <!-- Monitor Page -->
                                <a href="{{ route('whatsapp.campaigns.process', $campaign->id) }}" class="w-8 h-8 rounded-lg bg-blue-500/10 text-blue-500 hover:bg-blue-500 hover:text-white flex items-center justify-center transition-all shadow-sm hover:shadow-blue-500/30" title="شاشة المراقبة">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                                ...
                                <!-- Delete -->
                                <form action="{{ route('whatsapp.campaigns.destroy', $campaign->id) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من حذف هذه بيانت الحملة؟');">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="w-8 h-8 rounded-lg bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white flex items-center justify-center transition-all shadow-sm hover:shadow-red-500/30" title="حذف">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
        ...
        <div class="px-6 py-4 border-t border-gray-800 bg-gray-900/30">
            {{ $campaigns->links() }}
        </div>
        @endif
    </div>
