...
@section('title', 'تعديل قالب رسائل')

@section('header_title', 'تعديل قالب الرسائل: ' . $message->name)

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="bg-gray-900 rounded-xl border border-gray-800 p-6">
        <form action="{{ route('whatsapp.messages.update', $message->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ $message->name }}" class="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-white focus:ring-2 focus:ring-indigo-500 transition-all" required>
                </div>

                <!-- Messages (Dynamic List) -->
                <div x-data="{ messages: {{ json_encode($message->content) }} }">
                    <label class="block text-sm font-medium text-gray-400 mb-2">نصوص الرسائل (التبديل العشوائي)</label>
                    <div class="space-y-4">
                        <template x-for="(msg, index) in messages" :key="index">
                            <div class="relative">
                                <textarea :name="'content[' + index + ']'" rows="3" x-model="messages[index]" class="w-full bg-gray-800 border border-gray-700 rounded-lg px-4 py-3 text-white focus:ring-2 focus:ring-indigo-500 transition-all pr-10" required></textarea>
