...
@section('title', 'إنشاء قالب رسائل | Etman SMM')
...
<div class="max-w-3xl mx-auto direction-rtl">
    ...
    <div class="bg-[#1e1e24]/60 backdrop-blur-md border border-gray-800/80 rounded-3xl p-8 relative overflow-hidden shadow-2xl">
        ...
        <div class="flex items-center gap-4 mb-8 relative z-10">
            ...
            <div>
                <h2 class="text-2xl font-bold text-white">إنشاء قالب رسائل</h2>
                <p class="text-gray-400 text-sm">أضف نصوصاً جاهزة لاستخدامها في حملاتك المتعددة</p>
            </div>
        </div>

        <form action="{{ route('whatsapp.messages.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8 relative z-10">
            @csrf

            <!-- Name -->
            <div class="group">
                <label class="block text-sm font-bold text-gray-400 mb-2 transition-colors group-hover:text-green-500">اسم القالب</label>
                <input type="text" name="name" class="w-full bg-[#16161a]/80 border border-gray-700/50 rounded-xl px-4 py-4 text-white focus:ring-2 focus:ring-green-500/50 focus:border-green-500 transition-all placeholder-gray-600 hover:bg-[#16161a]" placeholder="مثال: رسائل الترحيب للعملاء الجدد" required>
            </div>
                ...
                <div class="space-y-4">
                    <template x-for="(msg, index) in messages" :key="index">
                        <div class="relative group">
                            ...
                            <textarea :name="'content[' + index + ']'" rows="3" x-model="messages[index]" placeholder="نص الرسالة..." class="w-full bg-[#16161a]/80 border border-gray-700/50 rounded-xl px-4 py-3 text-white focus:ring-2 focus:ring-green-500/50 focus:border-green-500 transition-all resize-y min-h-[100px] hover:bg-[#16161a]"></textarea>

                            <!-- Delete Button -->
                            <button type="button" @click="messages.splice(index, 1)" x-show="messages.length > 1" class="absolute top-2 left-2 text-red-500 hover:text-red-400 bg-red-500/10 p-1.5 rounded-lg transition-colors">
